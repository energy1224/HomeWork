package com.iakovenko.javacore.lesson20;
    ...
    public class Task01 {
        public static void main(String[] args) {
            ...
            double average = students.stream()
                    .collect(Collectors.averagingDouble(student -> LocalDate.now().getYear() - student.getBirth().getYear()));
            System.out.println(average);

            //  3. Найти самого младшего ученика - Stream -> min(Comparator<Student>): int compare(T o1, T o2); optional

            Comparator<Student> birth = (s1, s2) -> s2.getBirth().compareTo(s1.getBirth());
            Optional<Student> youngStudent = students.stream()
                    .min(birth);

            System.out.println(youngStudent.get());
...
//        Comparator<Student> studentComparator = byGender.thenComparing(byBirth);
            ...
            List<Student> listBySomeName = students.stream()
                    .filter(student -> student.getName().equals(someName))
                    .toList();

            System.out.println(listBySomeName);
